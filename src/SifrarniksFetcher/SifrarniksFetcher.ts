...
type SifrarnikType = 'proizvodnja' | 'knjigovodstvo' | 'place' | 'standardni'
...
export class SifrarniksFetcher {
    ...
    constructor(
        private _sifrarnikType: SifrarnikType
    ) {
    ...
    public async fetchSifrarnikMetaData(tables?: string[]) {
        ...
        for (let index = 0; index < sifrarnikTables.length; index++) {
            ...
            const sifrarnikPrimaryKeys: SifrarnikPrimaryKey[] = primaryKeys.map((primaryKeyInfo: SifrarnikPrimaryKey) => ({
